...
Then('je vois une liste de projets, avec pour chaque projet les champs suivants:', async function (
  table
) {
  const fields = table.rawTable[0]

  const found = await Promise.all(
    fields.map((field) => this.page.waitForSelector(testId(`projectList-item-${field}`)))
  )
