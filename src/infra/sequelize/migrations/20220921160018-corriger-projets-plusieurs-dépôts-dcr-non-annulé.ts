  ...
  async up(queryInterface: QueryInterface, Sequelize: Sequelize) {
    ...
    for (const id of projetsId) {
      try {
        const évènements = await EventStore.findAll(
          {
            ...
            attributes: ['occurredAt', 'payload'],
          },
          { transaction }
        )

        const évènementsCibles = évènements.map(({ dataValues }) => dataValues)

        if (!évènementsCibles.length || évènementsCibles.length === 1) {
          return
        }

        const nouveauxÉvènements = évènementsCibles.slice(0, -1).map((évènement) => {
          const {
            occurredAt,
        ...
        await EventStore.bulkCreate(nouveauxÉvènements.map(toPersistance), { transaction })
      } catch (e) {
