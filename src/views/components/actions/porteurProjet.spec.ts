...
describe('porteurProjetActions', () => {
  ...
  describe('when project is "classé"', () => {
    it('should return an action array with the following actions: "Demander un délai", "Changer de producteur", "Changer de fournisseur", "Changer d\'actionnaire", "Changer de puissance"', () => {
      const fakeProject = makeFakeProject({
        isClasse: true,
        appelOffre: { changementProducteurPossibleAvantAchèvement: true },
      });
      ...
      expect(result).toEqual([
        ...
        },
        {
          ...
          link: `/laureat/${encodeURIComponent(
            `${fakeProject.appelOffre}#${fakeProject.periode}#${fakeProject.famille}#${fakeProject.numeroCRE}`,
          )}/abandon/demander`,
        },
      ]);
    });
