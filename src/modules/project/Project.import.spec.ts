...
describe('Project.import({ data, importId })', () => {
  ...
  describe('when the project is not new', () => {
    ...
    describe('when data contains a notification date', () => {
      ...
      describe('REFwhen notifiedOn is the same as before', () => {
        it('should not emit', () => {
          const project = UnwrapForTest(
            makeProject({
              projectId,
              history: [
                new LegacyProjectSourced({
                  payload: {
                    ...
                    numeroCRE,
                    potentielIdentifier: '',
                    content: { ...fakeProject, notifiedOn: new Date('2020-01-01').getTime() },
                  },
                }),
              ],
              getProjectAppelOffre,
          ...
          project.import({
            data: { ...fakeProject, notifiedOn: new Date('2020-01-01').getTime() },
            importId,
          })
      ...
      describe('REF when notifiedOn has changed', () => {
        const project = UnwrapForTest(
          makeProject({
            projectId,
            history: [
        ...
        project.import({
          data: { ...fakeProject, notifiedOn: new Date('2020-01-02').getTime() },
          importId,
        })
