...
import { DemandeRaccordementEntity } from '../raccordement.entity';
...
export const registerConsulterDossierRaccordementQuery = ({
  ...
  const handler: MessageHandler<ConsulterDossierRaccordementQuery> = async ({
    ...
    const result = await find<DemandeRaccordementEntity>(
      `demande-raccordement|${identifiantProjet.formatter()}#${référenceDossierRaccordement}`,
    );
