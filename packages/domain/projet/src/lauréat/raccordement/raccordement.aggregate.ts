...
import {
  ...
  ImpossibleTransmettreDCRProjetAbandonnéError,
  PlusieursGestionnairesRéseauPourUnProjetError,
  RaccordementDéjàExistantError,
...
export class RaccordementAggregate extends AbstractAggregate<RaccordementEvent> {
  ...
  // private get identifiantGestionnaireRéseau

  private get identifiantProjet(): IdentifiantProjet.ValueType {
    return this.#lauréat.projet.identifiantProjet;
  }

  private contientLeDossier(référence: RéférenceDossierRaccordement.RawType) {
  ...
  // RéférenceDossierRaccordement.RawType
  private dateModifiée({ référence }, date) {
  ...
  async attribuerGestionnaireRéseau({
    ...
    // hum je comprends pas trop ce truc, peut être pas besoin ?
    ...
    if (raccordementDéjàExistantPourLeProjet) {
      throw new RaccordementDéjàExistantError(this.identifiantProjet.formatter());
    }
  ...
  async transmettreDemandeComplèteDeRaccordement({
    ...
    // Possible de vérifier ça directement dans projet
  ...
  private applyRaccordementSuppriméEventV1(event: RaccordementSuppriméEvent) {
