...
> {
  ...
  async notifier({ attestation: { format }, notifiéLe, notifiéPar }: NotifierOptions) {
    ...
    await this.producteur.importer({
      ...
      numéroImmatriculation: this.projet.candidature.dépôt.numéroImmatriculation,
    });
    ...
    if (
      champsSupplémentaire.installateur ||
      champsSupplémentaire.typologieInstallation ||
      champsSupplémentaire.dispositifDeStockage
    ) {
