...
import { AbandonEntity, StatutPreuveRecandidature } from '..';
    ...
    return {
      résultats: result.items
        .map((item) => ({
          ...
          demandéeLe: item.recandidature?.preuve?.demandéeLe
            ? DateTime.convertirEnValueType(item.recandidature?.preuve?.demandéeLe)
