...
export const registerConsulterRecoursQuery = ({
  find,
}: ConsulterRecoursDependencies) => {
  const handler: MessageHandler<ConsulterRecoursQuery> = async ({
    ...
    const result = await find<RecoursEntity>(
      `recours|${identifiantProjet.formatter()}`
    );
    ...
    const accord: ConsulterRecoursReadModel["accord"] = result.accordAccordéLe
      ? {
          ...
          accordéPar: IdentifiantUtilisateur.convertirEnValueType(
            result.accordAccordéPar!
          ),
    ...
    const rejet: ConsulterRecoursReadModel["rejet"] = result.rejetRejetéLe
      ? {
          ...
          rejetéPar: IdentifiantUtilisateur.convertirEnValueType(
            result.rejetRejetéPar!
          ),
          réponseSignée: DocumentProjet.convertirEnValueType(
            identifiantProjet.formatter(),
