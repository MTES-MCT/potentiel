...
export const registerConsulterAbandonQuery = ({ find }: ConsulterAbandonDependencies) => {
  const handler: MessageHandler<ConsulterAbandonQuery> = async ({ identifiantProjetValue }) => {
    ...
    const accord: ConsulterAbandonReadModel['accord'] = result.accord
      ? {
          accordéLe: DateTime.convertirEnValueType(result.accord.accordéLe),
          accordéPar: Email.convertirEnValueType(result.accord.accordéPar),
          réponseSignée: DocumentProjet.convertirEnValueType(
            identifiantProjet.formatter(),
            TypeDocumentAbandon.abandonAccordé.formatter(),
            DateTime.convertirEnValueType(result.accord.accordéLe).formatter(),
            result.accord.réponseSignée.format,
          ),
        }
      : undefined;

    const rejet: ConsulterAbandonReadModel['rejet'] = result.rejet
      ? {
          rejetéLe: DateTime.convertirEnValueType(result.rejet.rejetéLe),
          rejetéPar: Email.convertirEnValueType(result.rejet.rejetéPar),
          réponseSignée: DocumentProjet.convertirEnValueType(
            identifiantProjet.formatter(),
            TypeDocumentAbandon.abandonRejeté.formatter(),
            DateTime.convertirEnValueType(result.rejet.rejetéLe).formatter(),
            result.rejet.réponseSignée.format,
          ),
        }
      : undefined;
