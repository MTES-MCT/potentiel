...
export const modifierDemandeComplèteHandler: Raccordement['modifierDemandeComplète'] = async (
  _,
  identifiantProjet,
  reference,
  body,
  ...
  const { nouvelleReference } = validate(
    'ModifierDemandeCompleteBodyMergePatchCreateOrUpdate',
    body,
  );

  if (!nouvelleReference) {
    throw new Error('Au moins un champ doit être renseigné');
  }
