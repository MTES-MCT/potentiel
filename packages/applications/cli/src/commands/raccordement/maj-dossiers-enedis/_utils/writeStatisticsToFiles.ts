...
export const writeStatisticsToFiles = async (statistics: Statistics) => {
  ...
  if (statistics.UnSeulDossierDeRaccordement.total > 0) {
    ...
    /**
     * Un seul dossier de raccordement : transmettre la date de mise en service
     */
    if (statistics.UnSeulDossierDeRaccordement.transmettreDateMiseEnService.erreurs.length > 0) {
      await writeFile(
        path.resolve(
          __dirname,
          '../logs/unSeulDossierDeRaccordement',
          'transmettreDateMiseEnService_erreurs.json',
        ),
        JSON.stringify(
          statistics.UnSeulDossierDeRaccordement.transmettreDateMiseEnService.erreurs,
          null,
          2,
        ),
      );
    }
    if (statistics.UnSeulDossierDeRaccordement.transmettreDateMiseEnService.succès.length > 0) {
      await writeFile(
        path.resolve(
          __dirname,
          '../logs/unSeulDossierDeRaccordement',
          'transmettreDateMiseEnService_succès.json',
        ),
        JSON.stringify(
          statistics.UnSeulDossierDeRaccordement.transmettreDateMiseEnService.erreurs,
          null,
  ...
  if (statistics.pasDeDossierDeRaccordement.total > 0) {
     ...
     */
    if (statistics.pasDeDossierDeRaccordement.transmettreDateMiseEnService.erreurs.length > 0) {
      await writeFile(
        path.resolve(
          __dirname,
          '../logs/pasDeDossierDeRaccordement',
        ...
        JSON.stringify(
          statistics.pasDeDossierDeRaccordement.transmettreDateMiseEnService.erreurs,
          null,
          2,
        ),
      );
    }
    if (statistics.pasDeDossierDeRaccordement.transmettreDateMiseEnService.succès.length > 0) {
      await writeFile(
        path.resolve(
          __dirname,
          '../logs/pasDeDossierDeRaccordement',
        ...
        JSON.stringify(
          statistics.pasDeDossierDeRaccordement.transmettreDateMiseEnService.succès,
          null,
          2,
        ),
      );
