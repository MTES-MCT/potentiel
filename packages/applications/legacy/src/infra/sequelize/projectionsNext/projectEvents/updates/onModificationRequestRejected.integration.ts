...
import { File, ModificationRequest, Project, ProjectEvent } from '../..';
...
describe('onModificationRequestRejected', () => {
  ...
  it('should create a new project event of ModificationRequestRejected type even if no responseFileId is provided', async () => {
    await Project.create(makeFakeProject({ id: projectId }));
    await ModificationRequest.create(
      makeFakeModificationRequest({ id: modificationRequestId, projectId }),
    );
