...
SELECT
    ...
    MAX(CASE WHEN j.key = 'demande.recandidature.statut' THEN j.value END) AS recandidature_statut,
    MAX(CASE WHEN j.key = 'demande.recandidature.preuve.identifiantProjet' THEN j.value END) AS recandidature_preuve_identifiant_projet,
    MAX(CASE WHEN j.key = 'demande.recandidature.preuve.demandéeLe' THEN TO_TIMESTAMP(j.value, 'YYYY-MM-DD"T"HH24:MI:SS') END) AS recandidature_preuve_demandé_le,
    MAX(CASE WHEN j.key = 'demande.recandidature.preuve.transmiseLe' THEN TO_TIMESTAMP(j.value, 'YYYY-MM-DD"T"HH24:MI:SS') END) AS recandidature_preuve_transmise_le,
    MAX(CASE WHEN j.key = 'demande.recandidature.preuve.transmisePar' THEN j.value END) AS recandidature_preuve_transmise_par,
