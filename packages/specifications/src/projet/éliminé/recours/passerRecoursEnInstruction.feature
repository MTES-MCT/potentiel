# language: fr
@recours
Fonctionnalité: Passer la demande de recours d'un projet éliminé en instruction

    Contexte:
        Etant donné le projet éliminé "Du boulodrome de Marseille" avec :
            | appel d'offres | PPE2 - ZNI |

    Scénario: un utilisateur dgec passe le recours d'un projet éliminé en instruction
        Etant donné une demande de recours en cours pour le projet éliminé
        Quand l'utilisateur dgec passe en instruction le recours pour le projet éliminé
        Alors la demande de recours du projet éliminé devrait être en instruction
        Et un email a été envoyé au porteur avec :
            | sujet      | Potentiel - Du boulodrome de Marseille - Recours passé en instruction |
            | nom_projet | Du boulodrome de Marseille                                            |
            | url        | https://potentiel.beta.gouv.fr/elimines/.*/recours                    |

    Scénario: un utilisateur dgec reprend l'instruction du recours du projet éliminé
        Etant donné une demande de recours en instruction pour le projet éliminé
        Quand un nouvel utilisateur DGEC passe en instruction le recours pour le projet éliminé
        Alors la demande de recours du projet éliminé devrait être en instruction

    Scénario: Impossible de passer le recours d'un projet éliminé en instruction si le recours a déjà été accordé
        Etant donné une demande de recours accordée pour le projet éliminé
        Quand l'utilisateur dgec passe en instruction le recours pour le projet éliminé
        Alors le DGEC validateur devrait être informé que "Le recours a déjà été accordé"

    Scénario: Impossible de passer le recours d'un projet éliminé en instruction si le recours a déjà été rejeté
        Etant donné une demande de recours rejetée pour le projet éliminé
        Quand l'utilisateur dgec passe en instruction le recours pour le projet éliminé
        Alors le DGEC validateur devrait être informé que "Le recours a déjà été rejeté"

    Scénario: Impossible de passer le recours d'un projet éliminé en instruction si aucun recours n'a été demandé
        Quand l'utilisateur dgec passe en instruction le recours pour le projet éliminé
        Alors le DGEC validateur devrait être informé que "Aucun recours n'est en cours"

    Scénario: Impossible de reprendre le recours d'un projet éliminé en instruction si on instruit déjà le recours
        Etant donné une demande de recours en instruction pour le projet éliminé
        Quand le même utilisateur dgec passe en instruction le recours pour le projet éliminé
        Alors le DGEC validateur devrait être informé que "Le recours est déjà en instruction avec le même utilisateur dgec"

    Scénario: Impossible de passer le recours d'un projet éliminé en instruction si le recours a déjà été annulé
        Etant donné une demande de recours annulée pour le projet éliminé
        Quand l'utilisateur dgec passe en instruction le recours pour le projet éliminé
        Alors l'utilisateur dgec devrait être informé que "Le recours a déjà été annulé"
