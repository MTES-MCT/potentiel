      ...
      await mediator.send<GarantiesFinancières.SoumettreDépôtGarantiesFinancièresUseCase>(
        {
          ...
          data: {
            ...
            attestationValue: {
              content: convertStringToReadableStream(contenuFichier),
              format,
            },
            ...(dateÉchéance && {
              dateÉchéanceValue: new Date(dateÉchéance).toISOString(),
            }),
          },
        }
      );
...
Quand(
  "un admin supprime les garanties financières validées pour le projet {string}",
  async function (this: PotentielWorld, nomProjet: string) {
    try {
      const { identifiantProjet } =
        this.lauréatWorld.rechercherLauréatFixture(nomProjet);

      await mediator.send<GarantiesFinancières.SupprimerGarantiesFinancièresUseCase>(
        {
          type: "Lauréat.GarantiesFinancières.UseCase.SupprimerGarantiesFinancières",
          data: {
            identifiantProjetValue: identifiantProjet.formatter(),
            suppriméLeValue: new Date().toISOString(),
            suppriméParValue: "admin@test.test",
          },
        }
      );
      await sleep(500);
