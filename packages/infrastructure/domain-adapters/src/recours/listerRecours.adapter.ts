...
export const listerRecoursAdapter: Recours.ListerRecoursPort = async ({
  where,
  pagination,
  région,
}) => {
  ...
  const query = `${getRecoursQuery} ${whereClause} ${régionClause} order by value->>'misÀJourLe' desc ${paginationClause}`;

  const result = await executeSelect<{
    value: Recours.RecoursEntity;
  }>(query, ...(where ? Object.values(where) : []).concat(région ? [région] : []));

  const countQuery = `${countRecoursQuery} ${whereClause} ${régionClause}`;
