...
GRANT SELECT,
  INSERT,
  UPDATE,
  DELETE ON ALL TABLES IN SCHEMA public TO potentiel;
...
GRANT USAGE,
  SELECT,
  UPDATE ON SEQUENCE "public"."statistiquesUtilisation_id_seq" TO potentiel;
GRANT USAGE,
  SELECT,
  UPDATE ON SEQUENCE "public"."taches_id_seq" TO potentiel;
GRANT USAGE,
  SELECT,
  UPDATE ON SEQUENCE "public"."userDreals_id_seq" TO potentiel;

-- Event Store
GRANT USAGE ON SCHEMA event_store TO potentiel;
GRANT SELECT,
  INSERT ON event_store.event_stream TO potentiel;
GRANT SELECT,
  INSERT,
  UPDATE,
  DELETE ON event_store.subscriber TO potentiel;
GRANT SELECT,
  INSERT,
  UPDATE,
  DELETE ON event_store.pending_acknowledgement TO potentiel;
...
GRANT SELECT,
  INSERT,
  UPDATE,
  DELETE ON ALL TABLES IN SCHEMA auth TO potentiel;
GRANT USAGE,
  SELECT ON ALL SEQUENCES IN SCHEMA auth TO potentiel;
...
GRANT SELECT,
  INSERT,
  UPDATE,
  DELETE ON ALL TABLES IN SCHEMA domain_views TO potentiel;
ALTER DEFAULT PRIVILEGES IN SCHEMA domain_views
GRANT SELECT,
  INSERT,
  UPDATE,
  DELETE ON TABLES TO potentiel;
...
GRANT SELECT,
  INSERT,
  DELETE ON ALL TABLES IN SCHEMA domain_public_statistic TO potentiel;
ALTER DEFAULT PRIVILEGES IN SCHEMA domain_public_statistic
GRANT SELECT,
  INSERT,
  DELETE ON TABLES TO potentiel;
